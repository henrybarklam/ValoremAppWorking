...
struct ForgotPasswordView: View {
    @State var user: UserViewModel = UserViewModel() //verification on email field
    @Environment(\.presentationMode) var presentationMode //allows dismissal of sheet with button
    ...
    var body: some View {
        NavigationView {
            ...
            .alert(isPresented: $showAlert) {
                Alert(title: Text("Passowrd Reset"), message: Text(self.errString ?? "Success. Reset email sent successfully. Check your email."), dismissButton: .default(Text("OK")){
                    self.presentationMode.wrappedValue.dismiss()
                })
            }
        }
